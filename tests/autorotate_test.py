...
def test_process():
    ...
    pages = [Page(Path('{0:03}.jpg'.format(idx))) for idx in range(4)]
    ...
    with mock.patch('spreadsplug.autorotate.ProcessPoolExecutor') as mockctx:
        ...
        # function to call
        assert sorted([str(p.raw_image) for p in pages]) == (
